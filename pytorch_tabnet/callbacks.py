...
class EarlyStopping(Callback):
    ...
    def on_train_end(self, logs=None):
        ...
        if self.stopped_epoch > 0:
            msg = f"\nEarly stopping occured at epoch {self.best_epoch}"
            msg += (
                f" with best_{self.early_stopping_metric} = {round(self.best_loss, 5)}"
            )
            print(msg)
...
class History(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        msg += (
            f"|  {str(datetime.timedelta(seconds=self.total_time)) + 's':<6}"
        )
        print(msg)
